...
export default function NewsFeed() {
  const { onLazyLoad, data, currentSort, setCurrentSort, sortItems, allRows } =
    useLoadNews();
  ...
  return (
    <div className="flex flex-col gap-4 h-full p-4">
      <span className="font-bold block mb-2 text-center text-2xl">
        Список новостей
      </span>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <VirtualScroller
          items={allRows}
          itemSize={300}
          itemTemplate={(item: NewsItemType) => (
            <NewsItem item={item} onClick={selectNews} />
          )}
          lazy
